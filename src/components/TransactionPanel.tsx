...
export const TransactionPanel: React.FC<TransactionPanelProps> = ({ activeUser, isAuthenticated, deviceMode }) => {
  ...
  return (
    <div className={`flex flex-col ${
    ...
    } bg-slate-950 border border-slate-850 rounded-2xl overflow-hidden shadow-2xl relative font-sans text-slate-100`}>
      <style>{`
        .pop-flash {
      ...
      {/* 1. Trawler Unloading Session bar */}
      <div className="bg-gradient-to-b from-slate-900 to-slate-950 border-b border-slate-850 p-4 shrink-0 space-y-4">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-bold rounded-full border transition flex items-center gap-1.5 cursor-pointer selection:hidden select-none ${
                  isActive
                    ...
                    : isCompleted
                    ? "bg-slate-900 text-slate-500 border-slate-850 line-through decoration-slate-600"
                    : "bg-slate-900 text-slate-300 border-slate-850 hover:bg-slate-850"
                }`}
              >
        ...
        {showSrcInput && (
          <div className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl flex flex-col sm:flex-row gap-2.5 items-center animate-slideDown shadow-lg">
            <input
              ...
              placeholder="Source Trawler Name (e.g. Robin Kaka)"
              className="w-full sm:flex-grow text-xs text-slate-200 bg-slate-900 p-2.5 rounded-lg border border-slate-800 focus:outline-none focus:ring-1 focus:ring-amber-500 font-sans"
              autoFocus
            />
            <input
              type="text"
              ...
              className="w-full sm:w-28 text-xs text-slate-200 bg-slate-900 p-2.5 rounded-lg border border-slate-800 focus:outline-none focus:ring-1 focus:ring-amber-500 font-mono"
            />
      ...
      <div className="flex-grow overflow-y-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 shrink-0">
          {[
            ...
            <div
              key={label}
              className={`bg-gradient-to-br from-slate-900 to-slate-950 border ${borderColor} rounded-xl py-2.5 px-1 text-center select-none shadow-sm`}
            >
              <div className="text-[8px] text-slate-500 font-extrabold uppercase font-sans tracking-widest">{label}</div>
        ...
        {activeTxns.length === 0 && (
          <div className="text-center py-20 text-slate-500 text-xs font-sans space-y-2 select-none">
            <div className="text-2xl animate-bounce">🎣</div>
            <div className="font-bold">No transactions found for this source.</div>
            <p className="text-[10.5px] text-slate-600 max-w-sm mx-auto">
        ...
        <div className="space-y-2 animate-fadeIn">
          {[...activeTxns].reverse().map((t) => {
            ...
            return (
              <div
                key={t.id}
                className="flex justify-between items-center bg-slate-900/40 border border-slate-850 rounded-xl p-3 shadow-inner hover:bg-slate-905 transition duration-150"
              >
                <div className="flex-grow">
                  <div className="text-xs md:text-sm font-extrabold text-slate-200 uppercase tracking-tight flex items-center flex-wrap gap-1.5 font-sans">
                    <span>{b?.nickname || "Direct Spot Cash"}</span>
                    {t.fish_type && (
                      <span className="text-[9.5px] uppercase font-bold text-sky-450 text-sky-400 bg-sky-500/10 px-1.5 py-0.5 rounded-full border border-sky-400/10">
                        {t.fish_type}
                      </span>
                    )}
                    {t.added_by && (
                      <span className="text-[9px] font-medium font-mono text-slate-500 lowercase">
                        ({t.added_by})
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {canEdit && (
                    <button
                      ...
                      className="p-1.5 bg-slate-950 border border-slate-800 hover:border-slate-750 text-slate-400 hover:text-white rounded-lg cursor-pointer transition text-[11px]"
                      title="Adjust Record"
                    >
                      ✏️
                    </button>
      ...
      <div className="shrink-0 border-t-2 border-slate-850 bg-slate-900 p-3 select-none flex flex-col gap-2.5 z-10 shadow-[0_-5px_20px_rgba(0,0,0,0.3)]">
          ...
          {[
            ...
            return (
              <div
                ...
                className={`p-2 rounded-xl transition duration-150 cursor-pointer flex flex-col justify-between select-none ${
                  isActive
                    ? "bg-slate-955 border-2 border-amber-500 bg-slate-950 shadow-inner scale-[1.01]"
                    : "bg-slate-950 border-2 border-slate-850/80 hover:border-slate-800"
                }`}
              >
                <div className="text-[8px] text-slate-500 font-bold uppercase tracking-wider font-sans select-none">{label}</div>
                <div className="text-lg md:text-xl font-bold font-mono text-slate-100 flex items-baseline justify-between pt-1">
                  <span>{val || <span className="text-slate-700">0</span>}</span>
        ...
        {parseFloat(weight) > 0 && parseFloat(price) > 0 && (
          <div className="bg-emerald-950/20 border border-emerald-900/40 text-emerald-400 p-2 rounded-xl flex justify-between items-center font-mono selection:hidden select-none px-4 shadow-sm">
        ...
        <div className="grid grid-cols-4 gap-1.5 shrink-0 select-none animate-fadeIn">
            ...
            <button
              ...
              className="py-2.5 text-sm md:text-base font-black font-mono bg-slate-950 hover:bg-slate-850 border border-slate-800 hover:border-slate-700 rounded-lg cursor-pointer"
            >
              7
            </button>
            <button
            ...
            <button
              ...
              className="py-2.5 text-sm font-extrabold bg-slate-950 hover:bg-slate-850 border border-slate-800 text-rose-500 rounded-lg cursor-pointer flex items-center justify-center font-mono"
            >
              ⌫
            </button>
            ...
            <button
              ...
              className="py-2.5 text-xs font-black bg-slate-950 hover:bg-slate-850 border border-slate-800 text-rose-500 rounded-lg cursor-pointer flex items-center justify-center"
            >
              C
            </button>
            ...
            <button
              ...
              className={`py-6 text-xs md:text-sm font-black text-white hover:opacity-90 rounded-lg cursor-pointer transition-all uppercase flex flex-col justify-center items-center shadow-lg gap-1 select-none ${
...
const BuyerPicker: React.FC<BuyerPickerProps> = ({ buyers, onSelect, onClose, onAddAndSelect }) => {
  ...
  return (
    <div className="inset-0 absolute bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-scaleUp">
        ...
        <div className="px-5 py-3 border-b border-slate-900 select-none">
          <input
            ...
            className="w-full text-xs font-semibold bg-slate-950 text-slate-300 p-2.5 rounded-xl border border-slate-850 focus:outline-none focus:border-amber-500"
          />
        </div>

        {/* Scrollable list content */}
        <div className="flex-grow overflow-y-auto p-4 divide-y divide-slate-850/60">
          {filtered.map((b) => {
            ...
            return (
              <div
                ...
                className="py-3 px-2 flex justify-between items-center cursor-pointer hover:bg-slate-850 rounded-xl transition duration-150 group"
              >
                  ...
                  {isEditingThis ? (
                    <div className="flex gap-1.5 items-center mt-1" onClick={(e) => e.stopPropagation()}>
                      <input
                        ...
                        className="bg-slate-950 text-slate-200 text-xs p-1.5 rounded border border-slate-800 font-sans w-full"
                        autoFocus
                      />
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        className="px-2 py-1 bg-slate-850 text-slate-400 text-[9.5px] rounded"
                      >
                        X
                      </button>
                    </div>
                    ...
                    <div>
                      <div className="text-xs font-extrabold text-slate-205 text-slate-200 group-hover:text-amber-500 transition-colors uppercase flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1 text-slate-500 hover:text-slate-300 text-[9.5px] font-normal lowercase font-sans cursor-pointer"
                ...
                <div className="flex items-center gap-1.5 select-none shrink-0" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div
                    ...
                    className="p-1.5 bg-slate-950 border border-slate-800 rounded-lg text-slate-400 group-hover:text-amber-500 group-hover:border-amber-500/30 transition shadow-inner"
                  >
          ...
          {filtered.length === 0 && (
            <div className="py-12 text-center text-slate-500 text-xs font-semibold select-none space-y-4">
              ...
              {search.trim().length > 0 && (
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-955 text-xs font-black rounded-lg transition duration-150 inline-flex items-center gap-1.5 shadow cursor-pointer select-none"
                >
  ...
  return (
    <div className="inset-0 absolute bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <form
        ...
        className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-scaleUp text-left"
      >
        <div className="px-5 py-4 bg-gradient-to-b from-slate-950 to-slate-900 border-b border-slate-810 border-b border-slate-800 flex justify-between items-center select-none">
          <div>
            <h3 className="font-extrabold text-sm uppercase text-slate-200">Adjust Recorded Sale</h3>
            <p className="text-[10px] text-slate-500">Edit or delete recorded auction entries</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="text-[11px] font-mono font-bold bg-slate-950 border border-slate-805 px-2 py-1 hover:bg-slate-850 text-slate-400 hover:text-white rounded-lg cursor-pointer"
          >
            ✕ CANCEL
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[10.5px] font-extrabold text-slate-500 uppercase font-sans tracking-wider block">Buyer Nickname</label>
            <select
              value={buyerId}
              onChange={(e) => setBuyerId(e.target.value)}
              required
              className="w-full text-xs text-slate-205 text-slate-200 bg-slate-950 border border-slate-800 rounded-xl p-3 outline-none focus:ring-1 focus:ring-amber-500"
            >
              {buyers.map((b) => (
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div className="space-y-1.5">
              <label className="text-[10.5px] font-extrabold text-slate-500 uppercase font-sans tracking-wider block">Weight (kg)</label>
              <input
                type="text"
                value={weight}
                ...
                placeholder="0.0"
                required
                className="w-full text-xs text-slate-200 bg-slate-950 border border-slate-800 rounded-xl p-3 outline-none focus:ring-1 focus:ring-amber-500 font-mono"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10.5px] font-extrabold text-slate-500 uppercase font-sans tracking-wider block">Rate per kg (₹)</label>
              <input
                type="text"
                value={pricePerKg}
          ...
          <div className="bg-slate-950/60 border border-slate-850 p-3 rounded-xl flex justify-between items-center text-xs font-mono select-none">
            <span className="text-slate-500">Recalculated Aggregate:</span>
            <span className="font-extrabold text-emerald-450 text-emerald-400">
        ...
        <div className="px-5 py-4 bg-slate-950 border-t border-slate-850 flex flex-col sm:flex-row gap-2 select-none justify-between items-center">
          {/* Delete triggers */}
          {!confirmDel ? (
            <button
              type="button"
              onClick={() => setConfirmDel(true)}
              className="w-full sm:w-auto px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500 hover:text-white border border-rose-900/30 text-rose-400 rounded-xl text-xs font-bold transition cursor-pointer flex items-center justify-center gap-1 shrink-0"
            >
            ...
            <div className="flex gap-1.5 items-center w-full sm:w-auto">
              ...
              <button
                type="button"
                onClick={onDelete}
                className="px-2.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-[10.5px] font-extrabold cursor-pointer transition"
              >
                Delete
              </button>
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-900 border border-slate-800 text-slate-400 rounded-lg text-[10.5px] font-bold cursor-pointer transition"
              >
                Cancel
              </button>
            </div>
          ...
          <button
            type="submit"
            className="w-full sm:w-auto px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-xl text-xs tracking-wider cursor-pointer font-sans"
          >
            Save Changes
          </button>
        </div>
